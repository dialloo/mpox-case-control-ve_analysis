...
#            2. Estimate effectiveness by immunocompromising conditions and route of administmatch_ratio
...
# Load analysis dataset and output template ----------------------------------------------------------------------
  ...
  mpoxve <- here("data", "mmwr-publication-dataset", "mpoxve_matched_mmwr.RDS") %>% 
    read_rds() %>% 
...
# Stratified VE estimates: route of administration ------------------------------------------------------------------
  ...
  #==== Fully vaccinated ----
    analysis_full_route <- mpoxve %>% 
      filter(model == "Main" & !is.na(fully_adminroute_mod))
  ...
  #==== Partially vaccinated ----
   analysis_partial_route <- mpoxve %>% 
      filter(model == "Main" & !is.na(partially_adminroute_mod))
...
# Combine model results ------------------------------------------------------------------
  ...
  #==== Case control counts estimates ----
    ...
    ve_tidy_tbl2 <- ve_tidy_tbl %>% 
      ...
      select(results, Case, Control, Crude, Adjusted)
    
    #Anna: Use the clipr output. Run and past it into excel before
    #      pasting it into word.
    clipr::write_clip(ve_tidy_tbl2)
    
    #write.csv(ve_tidy_tbl2, file = "results_output/tbl_ve-results.csv", na = "")
