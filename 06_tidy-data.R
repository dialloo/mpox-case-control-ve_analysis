...
# Tidy data -------------------------------------------------------------------------------------------------------------
  mpoxve <- raw %>%    
    ...
    mutate( #! Variable name key: 
            ...
            redcap_trans_date       = ymd("2023-03-28"),
            
            mmwr_data_closing_date  = ymd("2022-04-31"),
  ...
  #==== Output match dataset for 1:4 match_ratio: main model & s6 full vaccinated ----
    ...
    m_data_r4 <- match.data(matchit(I(caco == "Case") ~ index_date, 
      ...
      mutate(match_ratio = "1 case:4 controls",
             match_vars = "State & IE",
             model = "Main")
  #==== Output match dataset for 1:4 match_ratio: main model & s6 excluded ----
    # IE (caliper)  & State (Exact)
  ...
  #==== combine the data to create analysis dataset ----------------------------------
    ...
    mpoxve_matched <-  mpoxve_main %>% 
      ...
      select(model, match_ratio, match_vars, match_ratio_exact, pid, caco, matched_id, match_weights,
             index_date, vax_stat_ve_main, vax_stat_ve_s6full, vax_stat_ve_s6excl,
             sid, jid, age, age_cat, race_ethn_cat, race_ethn, elig_gender_cat, health_ins_cat,
             ...
             fully_adminroute_cat, partially_adminroute_cat,
             everything()) %>% 
    ...
    #==== summary of matched data ----
      ...
      r_upto<- mpoxve_matched %>%
        pfreq(model, match_ratio, match_vars, sid, caco) %>%
     ...
     r_exact <- mpoxve_matched %>%
      ...
      caco_match_counts <- bind_rows(r_upto, r_exact) %>% 
        select(model, match_ratio, match_vars, match_data, sid, everything()) %>% 
      ...
      caco_match_counts_r <- caco_match_counts %>%
        ...
        full_join((caco_match_counts %>%
                     ...
                     ungroup() %>% 
                     rename(tot = n) %>% 
                     select(model, sid, caco, tot)), 
                  by = c("model", "sid", "caco")) %>% 
        mutate(pct_tot = round(100*(n/tot), 1)) %>% 
        filter(match_ratio == "1 case:4 controls" & match_vars == "State & IE" & match_data == "Upto") %>% 
        ungroup() %>% 
        select(model, sid, caco, n, tot, pct_tot) %>% 
        mutate(model = case_when(model == "Main" ~ " M1-S6 partial &\nM2-S6 full",
